...
class Documentation(Command):
    ...
    user_options = [
        ...
        ('dist-dir=', None, 'directory to put final built distributions in'
                            ' [default: dist/docs]'),
        ('build-dir=', None, 'temporary directory for the distribution'
                             ' [default: build/docs]'),
...
class BakedDocumentation(Command):
    """Build the doc of a baked project."""
...
def working_directory(path):
    """Change the working directory to the given path and revert on exit."""
    prev_cwd = os.getcwd()
    os.chdir(str(path))
    try:
        yield
...
def main():
    """Main entry, calls :func:`setuptools.setup`."""
