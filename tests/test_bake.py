...
def test_bake_selecting_license(cookies):
    ...
    for name, target_string in license_strings.items():
        with bake_in_temp_dir(cookies, extra_context={'project_license': name}) as result:
            assert target_string in result.project.join('LICENSE').read()
            assert name in result.project.join('setup.py', '__main__.py').read()


def test_bake_not_open_source(cookies):
    with bake_in_temp_dir(cookies, extra_context={'project_license': 'Not open source'}) as result:
        found_toplevel_files = [f.basename for f in result.project.listdir()]
        assert 'setup.py' in found_toplevel_files
...
def test_bake_with_console_script_files(cookies):
    ...
    setup_path = os.path.join(project_path, 'setup.py', '__main__.py')
